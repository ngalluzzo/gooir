...
explicit trust policy, whose default admits no claim bindings.
...
and execution metadata. Before admission, the host is responsible for validating
its authenticity, the attester's authority, and the referenced artifact. The
policy then binds that result to one exact operation identity and semantic claim,
including contract, payload, source provenance, status, and conformance record.
An admitted tuple is therefore not a reusable bearer token for other claims.
This slice models auditable admission, not cryptographic proof or a global
authority registry.
...
Exact contextual claim admission gives the first externally lifted analyses a
default-deny boundary while keeping authority governance, signature algorithms,
and semantic meaning outside the microkernel. Binding operation identity and
claim contents also prevents a plugin from copying a valid conformance tuple
onto a different assertion.
