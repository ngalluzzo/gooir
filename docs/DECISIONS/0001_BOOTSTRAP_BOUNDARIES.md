...
- Accept a claim as trusted in the first analysis only when it carries verified status plus conformance-suite evidence. Declared, inferred, observed, missing, conflicting, or invalidly projected claims remain unknown for safety analysis. This provisional rule is superseded by [decision 0002](0002_EVIDENCE_TRUST_POLICY.md): evidence must also be admitted by an explicit contextual trust policy.
...
- Revisit trust composition only with concrete conflicting/observed claim cases. The first concrete policy is recorded in [decision 0002](0002_EVIDENCE_TRUST_POLICY.md).
