...
const PROXY_IO_DEADLINE: Duration = Duration::from_secs(15);
...
fn run_proxy(mut arguments: impl Iterator<Item = OsString>) {
    ...
    assert_eq!(first_request.target, "/v1/direct-conversations");
    ...
    first_request
        ...
        .unwrap_or_else(|_| panic!("crash proxy could not close withheld client connection"));
    ...
    assert_eq!(replay_request.target, "/v1/direct-conversations");
    assert_eq!(replay_response.status, 200);
    replay_request
        .client
        .write_all(&replay_response.bytes)
        .unwrap_or_else(|_| panic!("crash proxy replay forwarding failed"));
    replay_request
        .client
        .shutdown(std::net::Shutdown::Write)
        .unwrap_or_else(|_| panic!("crash proxy replay shutdown failed"));
    ...
    assert_eq!(attester_request.method, "GET");
    assert_eq!(
        attester_request.target,
        "/v1/conversations?include_archived=true"
    );
    assert_eq!(attester_response.status, 200);
    attester_request
        .client
        .write_all(&attester_response.bytes)
        .unwrap_or_else(|_| panic!("crash proxy attestation forwarding failed"));
    attester_request
        .client
        .shutdown(std::net::Shutdown::Write)
        .unwrap_or_else(|_| panic!("crash proxy attestation shutdown failed"));
...
impl ProxyResponse {
    fn observation(&self) -> ProxyObservation {
        ...
        ProxyObservation {
            status: self.status,
            body_bytes: u64::try_from(body.len()).expect("proxy body length fits u64"),
            body_digest: sha256_identity(body),
        }
    }
}
...
fn proxy_exchange(
    ...
    let (client, request_bytes) = (0..4)
        .find_map(|_| {
            let (mut client, peer) = listener
                .accept()
                .unwrap_or_else(|_| panic!("crash proxy accept failed"));
            assert!(peer.ip().is_loopback());
            configure_proxy_stream(&client);
            read_framed_http(&mut client, HttpSide::Request).map(|request| (client, request))
        })
        .unwrap_or_else(|| panic!("crash proxy empty-connection capacity was exhausted"));
    let request = parse_request_head(&request_bytes);
    let request_body = &request_bytes[request.2..];

    let mut upstream = std::net::TcpStream::connect_timeout(&backend, PROXY_IO_DEADLINE)
        .unwrap_or_else(|_| panic!("crash proxy backend connect failed"));
    configure_proxy_stream(&upstream);
    upstream
        .write_all(&request_bytes)
        .unwrap_or_else(|_| panic!("crash proxy backend request write failed"));
    let response_bytes = read_framed_http(&mut upstream, HttpSide::Response)
        .unwrap_or_else(|| panic!("crash proxy backend closed before its response"));
    let (status, body_offset) = parse_response_head(&response_bytes);
    (
        ProxyRequest {
            client,
            method: request.0,
            target: request.1,
            body_bytes: u64::try_from(request_body.len()).expect("proxy body length fits u64"),
            body_digest: sha256_identity(request_body),
        },
        ProxyResponse {
...
fn read_framed_http(stream: &mut std::net::TcpStream, side: HttpSide) -> Option<Vec<u8>> {
    let mut bytes = Vec::new();
    let mut buffer = [0_u8; 8 * 1024];
    let header_end = loop {
        let count = stream
            .read(&mut buffer)
            .unwrap_or_else(|_| panic!("crash proxy HTTP header read failed"));
        if count == 0 && bytes.is_empty() {
            return None;
        }
        assert_ne!(count, 0, "crash proxy HTTP message ended within its header");
        bytes.extend_from_slice(&buffer[..count]);
        assert!(
            bytes.len() <= MAX_HTTP_MESSAGE_BYTES,
            "crash proxy HTTP message exceeded its bound"
        );
    ...
    let expected = header_end
        ...
        .unwrap_or_else(|| panic!("crash proxy HTTP length overflowed"));
    assert!(expected <= MAX_HTTP_MESSAGE_BYTES);
    assert!(
        bytes.len() <= expected,
        "crash proxy rejected HTTP pipelining"
    );
    while bytes.len() < expected {
        let available = buffer.len().min(expected - bytes.len());
        let count = stream
            .read(&mut buffer[..available])
            .unwrap_or_else(|_| panic!("crash proxy HTTP body read failed"));
        assert_ne!(count, 0, "crash proxy HTTP message ended before its body");
        bytes.extend_from_slice(&buffer[..count]);
    }
    Some(bytes)
}

fn parse_request_head(bytes: &[u8]) -> (String, String, usize) {
    let mut headers = [httparse::EMPTY_HEADER; 64];
    let mut request = httparse::Request::new(&mut headers);
    let httparse::Status::Complete(body_offset) = request
        .parse(bytes)
        .unwrap_or_else(|_| panic!("crash proxy request header was malformed"))
    else {
        panic!("crash proxy request header was incomplete");
    };
    assert_eq!(request.version, Some(1));
    (
        request
            .method
            .unwrap_or_else(|| panic!("crash proxy request method was absent"))
            .to_owned(),
        request
            .path
            .unwrap_or_else(|| panic!("crash proxy request target was absent"))
            .to_owned(),
        body_offset,
    )
}

fn parse_response_head(bytes: &[u8]) -> (u16, usize) {
    let mut headers = [httparse::EMPTY_HEADER; 64];
    let mut response = httparse::Response::new(&mut headers);
    let httparse::Status::Complete(body_offset) = response
        .parse(bytes)
        .unwrap_or_else(|_| panic!("crash proxy response header was malformed"))
    else {
        panic!("crash proxy response header was incomplete");
    };
    assert_eq!(response.version, Some(1));
    (
        response
            .code
            .unwrap_or_else(|| panic!("crash proxy response status was absent")),
        body_offset,
    )
}
...
fn parse_request_content_length(bytes: &[u8]) -> usize {
    ...
    assert!(matches!(
        request
            .parse(bytes)
            .unwrap_or_else(|_| panic!("crash proxy request framing was malformed")),
...
fn parse_response_content_length(bytes: &[u8]) -> usize {
    ...
    assert!(matches!(
        response
            .parse(bytes)
            .unwrap_or_else(|_| panic!("crash proxy response framing was malformed")),
        httparse::Status::Complete(_)
    ));
...
fn content_length(headers: &[httparse::Header<'_>]) -> usize {
    assert!(
        ...
        "crash proxy rejects transfer encoding"
    );
    let lengths = headers
        .iter()
        ...
        .map(|header| {
            std::str::from_utf8(header.value)
                ...
                .unwrap_or_else(|| panic!("crash proxy content length was invalid"))
        })
        .collect::<Vec<_>>();
    match lengths.as_slice() {
        [] => 0,
        [length] => *length,
        _ => panic!("crash proxy rejected ambiguous content length"),
    }
}

fn configure_proxy_stream(stream: &std::net::TcpStream) {
    stream
        .set_read_timeout(Some(PROXY_IO_DEADLINE))
        .unwrap_or_else(|_| panic!("crash proxy read deadline failed"));
    stream
        .set_write_timeout(Some(PROXY_IO_DEADLINE))
        .unwrap_or_else(|_| panic!("crash proxy write deadline failed"));
}
