...
/// The waist carries no enum member names, so one placeholder type stands in.
pub const ENUM_TYPE: &str = "gooi_enumeration";
...
fn sql_type(ty: FieldType) -> Option<&'static str> {
    Some(match ty {
        FieldType::Scalar(s) => match s {
            ...
            ScalarType::Enumeration => ENUM_TYPE,
...
pub fn lower_to_postgres_ddl(model: &DataModel) -> Lowered {
    ...
    if needs_enum {
        out.lossy.push(Lossy {
            ...
            detail: "the waist carries no enum member names".to_owned(),
        });
        writeln!(s, "CREATE TYPE {ENUM_TYPE} AS ENUM ('PLACEHOLDER');").expect("string write");
    }
    ...
    for e in &model.entities {
        for f in e.fields.iter().filter(|f| f.unique) {
            writeln!(
                s,
